...
export function ReviewCard({ review, compact = false }: { review: Review; compact?: boolean }) {
  ...
  return (
    <article className={compact ? "card review-card review-card-compact" : "card review-card"}>
      <Link href={`/books/${review.book.slug}`}>
        <BookCover
      ...
      <div className="review-main">
          ...
          <div className="review-byline">
            <div className="user-row">
              ...
              <div className="stack" style={{ gap: 2 }}>
                <div>
                  <Link href={`/profile/${review.user.username}`} className="username-link">
                    @{review.user.username}
                  </Link>{" "}
                  <span className="muted">reviewed</span>{" "}
                  <Link href={`/books/${review.book.slug}`} className="book-title-link">
                    {review.book.title}
                  </Link>
                </div>
        ...
        <div className="review-actions">
          <div className="review-action-group">
            ...
            <Link href={`/books/${review.book.slug}?tab=reviews#write-review`} className="icon-action">
              <CommentIcon />
